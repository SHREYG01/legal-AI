...
export function getSummary(contractId: string): Promise<ApiSummaryResponse> {
  return request<ApiSummaryResponse>(`/api/contracts/${encodeURIComponent(contractId)}/summary`);
}
...
export function askQuestion(contractId: string, question: string): Promise<ApiAnswerResponse> {
  return request<ApiAnswerResponse>(`/api/contracts/${encodeURIComponent(contractId)}/ask`, {
    method: "POST",
